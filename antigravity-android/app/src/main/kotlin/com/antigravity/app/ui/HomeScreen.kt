...
/**
 * Home 画面 — 読み取り専用ダッシュボード。
 *
 * Phase 4 追加:
 *   - PullToRefreshBox: スワイプで手動更新
 *   - SnackbarHost: 更新完了 / 失敗フィードバック
 *   - 診断ボタン: TopAppBar に追加 → DiagnosticsScreen へ遷移
 */
...
fun HomeScreen(
    ...
    // snackbarMessage が非 null になったら表示して消す (one-shot)
    ...
    Card(
        ...
        Row(
            ...
            Column {
                ...
                Text(text = row.timeLabel, style = MaterialTheme.typography.bodySmall,
...
private fun FreshnessDot(level: FreshnessLevel) {
    val color = when (level) {
        FreshnessLevel.FRESH   -> Color(0xFF4CAF50)
        FreshnessLevel.LAGGING -> Color(0xFFFFA726)
        FreshnessLevel.STALE   -> MaterialTheme.colorScheme.error
        FreshnessLevel.UNKNOWN -> MaterialTheme.colorScheme.outline
    }
...
private fun EmptyState() {
    Box(
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            Text(text = "データなし", style = MaterialTheme.typography.titleMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant)
            Spacer(modifier = Modifier.height(8.dp))
            Text(text = "「更新」をタップしてデータを取得してください",
...
private fun HomeScreenPreview() {
    AntigravityTheme {
        HomeScreen(
            uiState = HomeUiState(
                quoteRows = listOf(
                    QuoteRowData("asset-gmopg", "GMO-PG", "9,920円", "現在値", FreshnessLevel.FRESH, true),
                    QuoteRowData("asset-unext", "U-NEXT", "3,450円", "4/5 終値", FreshnessLevel.STALE, false),
                ),
                fetchStatus = null, lastSyncAt = "2024-04-08T10:30:00+09:00", isLoading = false,
            ),
            onRefresh = {}, onDiagnostics = {}, snackbarMessage = null, onSnackbarShown = {},
        )
    }
}
...
private fun HomeScreenFallbackPreview() {
    AntigravityTheme {
        HomeScreen(
            uiState = HomeUiState(
                quoteRows = listOf(
                    QuoteRowData("asset-gmopg", "GMO-PG", "9,870円", "4/6 10:10時点", FreshnessLevel.LAGGING, false),
                ),
                ...
                lastSyncAt = "2024-04-06T10:10:00+09:00", isLoading = false,
            ),
            onRefresh = {}, onDiagnostics = {}, snackbarMessage = null, onSnackbarShown = {},
        )
    }
